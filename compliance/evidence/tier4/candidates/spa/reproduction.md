...
1. Run the external counterpart `spa-rp` against the `hardening` profile.
...
3. Supply the artifacts under an external root at `<external-root>/spa/`.
4. Re-run `python scripts/materialize_tier4_peer_evidence.py --external-root <external-root>` to normalize and validate the preserved bundle for `spa`.
